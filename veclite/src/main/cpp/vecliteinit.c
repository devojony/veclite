...
void app_log(const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    __android_log_vprint(ANDROID_LOG_INFO, "veclite", fmt, args);
    va_end(args);
}
...
static void l2d_func(sqlite3_context *context, int argc, sqlite3_value **argv) {
    ...
    app_log("执行l2d 函数 %s / %s ", (const char *) arg1, (const char *) arg2);
...
int sqlite3_extension_init(
    ...
    sqlite3_create_function(db, "l2d", 2, SQLITE_UTF8, 0, l2d_func, 0, 0);
//    sqlite3_create_function(db, "knn", 3, SQLITE_UTF8, 0, k_nearest_neighbors, 0, 0);
    return SQLITE_OK;
}